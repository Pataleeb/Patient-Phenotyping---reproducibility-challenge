...
def _prepare_text_data(df_tr, df_va, df_te, labels, seq_len, w2v_path, emb_dim):
    ...
    emb_matrix = None
    if w2v_path is not None and Path(w2v_path).exists():
        w2v, d = load_w2v_bin(str(w2v_path))
        emb_dim = d if emb_dim is None else emb_dim
        emb_matrix = build_embedding_matrix(vocab, w2v, emb_dim)
    else:
        if emb_dim is None:
            emb_dim = 128

    return txt_tr, txt_va, txt_te, ytr, yva, yte, J, vec, vocab, emb_matrix, emb_dim

def fit_eval_lstm(df_tr, df_va, df_te, labels, w2v_path=None, seq_len=600, emb_dim=None, epochs=3, batch=64):
    import tensorflow as tf
    ...
    txt_tr, txt_va, txt_te, ytr, yva, yte, J, vec, vocab, emb_matrix, emb_dim = _prepare_text_data(
        df_tr, df_va, df_te, labels, seq_len, w2v_path, emb_dim
    )
    ...
    model.fit(ds(txt_tr, ytr), validation_data=ds(txt_va, yva), epochs=epochs, verbose=2)
    ...
    return metrics_from_probs(yte, p_te, thr)

def fit_eval_gru(df_tr, df_va, df_te, labels, w2v_path=None, seq_len=600, emb_dim=None, epochs=3, batch=64):
    import tensorflow as tf
    from tensorflow.keras import layers, models
...
def to_wide_table(results: Dict[str, Dict[int, Dict[str, float]]], labels: List[str]):
    rows = []
    ...
    for j, name in enumerate(labels):
        ...
        for m in methods:
            ...
            if res is None or j not in res:
                ...
                row[f"{m}_P"] = 100 * res[j]["P"]
                row[f"{m}_R"] = 100 * res[j]["R"]
                row[f"{m}_F1"] = 100 * res[j]["F1"]
                row[f"{m}_AUC"] = 100 * res[j]["AUC"]
        rows.append(row)
...
def main():
    ...
    results["LSTM"] = fit_eval_lstm(df_tr, df_va, df_te, labels, w2v_path=str(W2V) if W2V.exists() else None, seq_len=600, epochs=3, batch=64)
    results["GRU"] = fit_eval_gru(df_tr, df_va, df_te, labels, w2v_path=str(W2V) if W2V.exists() else None, seq_len=600, epochs=3, batch=64)

    table = to_wide_table(results, labels)
    csv_path = OUT_DIR / "table_results_lstm_gru_bin.csv"
